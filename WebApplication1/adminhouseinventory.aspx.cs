        ...
        void getHouseByID()
        {
            try
            {
                ...
                if (dt.Rows.Count >= 1)
                {
                    ...
                    //DropDownList3.Items.FindByValue(dt.Rows[0]["appointment_id"].ToString().Trim()).Selected = true;
        ...
        void updateHouseByID()
        {
            if (checkHouseExists())
            {
                try
                {
                    ...
                    con.Close();
                    GridView1.DataBind();
                    Response.Write("<script>alert('House Listing Updated.');</script>");
                }
                catch (Exception ex)
                {
                    Response.Write("<script>alert('" + ex.Message + "');</script>");
        ...
        void getHouseByID()
        {
            try
            {
                ...
                if(dt.Rows.Count>=1)
                {
                    ...
                    DropDownList2.SelectedValue = dt.Rows[0]["agent_id"].ToString().Trim();
                    DropDownList3.SelectedValue = dt.Rows[0]["appointment_id"].ToString().Trim();
